...
class DictationTextPostprocessor:
    ...
    def process(
        ...
        *,
        ...
        if self.llm.enabled:
            ...
            try:
                llm_output = self._call_llm(llm_input, language=language, context=context)
    ...
    def _call_llm(
        ...
        *,
        ...
        user_prompt = self._render_user_prompt(text, language=language, context=context)
    ...
    def _render_user_prompt(
        ...
        *,
        ...
        template = self.llm.user_prompt_template or '{text}'
    ...
    def _build_context_block(self, context: DictationContext | None) -> str:
        ...
        if context.element_role:
            lines.append(f'role: {context.element_role}')
        if context.element_title:
            lines.append(f'element: {context.element_title}')
        if context.page_url:
        ...
        if context.context_text and context.context_text != context.selected_text:
            lines.extend(
                [
                    'context_excerpt:',
                    '<<<',
